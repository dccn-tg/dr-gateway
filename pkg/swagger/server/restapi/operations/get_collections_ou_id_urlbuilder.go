...
type GetCollectionsOuIDURL struct {
	ID string
...
func (o *GetCollectionsOuIDURL) Build() (*url.URL, error) {
	...
	var _path = "/collections/ou/{id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetCollectionsOuIDURL")
	}
...
func (o *GetCollectionsOuIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetCollectionsOuIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetCollectionsOuIDURL")
	}
